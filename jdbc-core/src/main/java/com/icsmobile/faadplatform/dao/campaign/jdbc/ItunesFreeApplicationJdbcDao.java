...
@Repository
public class ItunesFreeApplicationJdbcDao extends BaseDao implements IItunesFreeApplicationDao {
	...
	@Autowired
	public ItunesFreeApplicationJdbcDao(DataSource itunesAppDataSource) {
		this.setDataSource(itunesAppDataSource);
	}
	
	public ItunesFreeApplicationJdbcDao() {
	}
	...
	public List<ItunesApplication> findTotalMovementGreaterThan20(List<String> itunesAppId) {
		...
		cal.add(Calendar.HOUR, 0 - 4);
		String start = sdf.format(cal.getTime());
		
        StringBuilder q = new StringBuilder();
        for(String name :itunesAppId){
                q.append("?, ");
        }

		String sql = "select distinct * from itunes_application ia1 join (select itunes_app_id, sum(moved) as movement from itunes_application where pull_date <= ? and pull_date >= ? and price_type = 'Free' group by itunes_app_id) as ia2 where ia1.itunes_app_id = ia2.itunes_app_id and ia2.movement >= 20 and ia1.itunes_app_id in (" + q.replace(q.lastIndexOf(","), q.length(), "") + ") group by ia1.itunes_app_id order by ranking asc";

        List<Object> params = new ArrayList<Object>();
        params.add(now);
        params.add(start);
        for(int i = 0 ; i < itunesAppId.size(); i ++){
                params.add(itunesAppId.get(i));
        }   	
	...
	public List<ItunesApplication> findHotApps(String date, int moved, int hours) {
		...
		String sql = QUERY + " where moved >= ? group by ia1.itunes_app_id order by ia1.ranking asc";
